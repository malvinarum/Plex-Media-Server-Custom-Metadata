export default {
  async fetch(request, env, ctx) {
    ...
    // -------------------------------------------------------------------------
    // FIX: Must return "MediaProvider" (not MediaContainer) to be valid.
    if (path === '/') {
      ...
      return json({
        MediaProvider: {
          ...
          version: "1.0.0",
          // FIX: Only expose supported types (1=Movie, 2=Show, 3=Season, 4=Episode)
          // Hiding 8 (Artist) and 9 (Album) prevents the "Unsupported metadata types" error.
          Types: [
            { type: 1, Scheme: [{ scheme: AGENT_ID }] }, 
            { type: 2, Scheme: [{ scheme: AGENT_ID }] }, 
            { type: 3, Scheme: [{ scheme: AGENT_ID }] }, 
            { type: 4, Scheme: [{ scheme: AGENT_ID }] }  
          ],
          Feature: [
            { type: "search", key: "/search" },
            { type: "metadata", key: "/metadata" },
            { type: "match", key: "/search" } // Match reuses search logic but uses POST
          ]
        }
      });
    }
    ...
    try {
      ...
      if (path === '/search' || path === '/plex/search') {
        ...
        // --- 📨 HANDLE ARGUMENTS (GET vs POST) ---
        // Search uses GET. Match uses POST. We need to handle both.
        let query, year, typeStr;

        if (request.method === 'POST') {
          try {
            const body = await request.json();
            query = body.title || body.query; // 'match' sends 'title'
            year = body.year;
            typeStr = body.type;
          } catch (e) {
        ...
        // Normalize Type (Plex sends integers for Match, strings for Search sometimes)
        let type = 'unknown';
        ...
        // --- MUSIC/BOOKS (Hidden from manifest, but logic kept for future) ---
      ...
      if (path === '/metadata' || path === '/plex/metadata') {
        // Plex might send ID in 'ratingKey', 'id', or 'guid' params
